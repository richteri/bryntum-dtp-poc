 ...
 * @Last Modified time: 2019-12-16 11:03:27
 */
StartTest(t => {
    t.it('Rendering', t => {
        t.chain(
            { waitForSelector : '.b-gantt', desc: 'Should have gantt' },
            { waitForSelector : '.b-gantt-task', desc: 'Should have task' },
            { waitForSelector : '.b-panel .b-grid', desc: 'Should have grid in panel' }
        );
    });
    ...
    t.it('Interaction', t => {
        t.chain(
            { waitForSelector : '.b-sch-timerange label:textEquals(Critical milestone)', desc: 'Should have time range' },
            { click : '.b-button:contains(Show header elements)' },
            { waitForSelectorNotFound : 'b-sch-timerange label:textEquals(Critical milestone)', desc: 'Shouldn\'t have time range' },
            { click : '.b-button:contains(Show header elements)' },
            { dblclick : '.b-grid-cell:textEquals(Critical milestone)' },
            { type : 'My milestone[ENTER]' },
            { waitForSelector : '.b-sch-timerange label:textEquals(My milestone)', desc: 'Can edit time range' },
            { click : 'button:contains(Add)' },
            { waitForSelector : '.b-sch-timerange label:textEquals(New range)', desc: 'Can create time range' }
