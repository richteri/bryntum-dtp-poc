...
export default class GridBase extends base(Container).mixes(
    ...
    static getLKey() {
        return '8fc784b3-3eb2-11ea-86f5-d094663d5c88';
    }
    ...
    bindStore(store) {
        ...
        if (store) {
            store.on({
                ...
                idChange      : me.onStoreRecordIdChange,
        ...
        if (me.activeMask) {
            ...
            switch (type) {
                ...
                case 'exception':
                    ...
                    break;
                default:
                    message = response.status && response.statusText ? `${response.status} - ${response.statusText}` : me.L('L{unknownFailure}');
            }

            me.activeMask.icon = me.loadMaskErrorIcon;
            me.activeMask.text = `<div class="b-grid-load-failure">
                <div class="b-grid-load-fail">${action === 'read' ? me.L('L{loadFailedMessage}') : me.L('L{syncFailedMessage}')}</div>
        ...
        Object.entries(subGrids).forEach(([region, subGrid]) => {
            ...
            if (column.draggable &&
                ...
                const moveRight = subGrid.element.compareDocumentPosition(subGrids[column.region].element) === document.DOCUMENT_POSITION_PRECEDING,
                    ...
                    text = regions.length > 2  ? me.L('L{moveColumnTo}', me.L(region)) : me.L(moveRight ? 'L{moveColumnRight}' : 'L{moveColumnLeft}');
...
VersionHelper.setVersion('grid', '3.1.9');
