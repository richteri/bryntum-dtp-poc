...
const
    ...
    getNewYear          = (date, yearDelta = 0) => new Date(date.getFullYear() + yearDelta, 0, 1),
    getNewYearDayNumber = (date, newYear) => Math.floor((date.getTime() - newYear.getTime() -
        (date.getTimezoneOffset() - newYear.getTimezoneOffset()) * validConversions.minute.millisecond) / validConversions.day.millisecond) + 1,
...
export default class DateHelper extends Localizable() {
     ...
     * @category Query
     */
    static getWeekNumber(date, weekStartDay = DateHelper.weekStartDay, yearDelta = 0) {
        let year      = getNewYear(date, yearDelta),
            dayNumber = getNewYearDayNumber(date, year);
        const firstDay = normalizeDay(year.getDay() - DateHelper.weekStartDay);
        let weekNumber;
        // Check if the year starts before the middle of a week
        if (firstDay < 4) {
            weekNumber = Math.floor((dayNumber + firstDay - 1) / 7) + 1;
            if (weekNumber > 52) {
                year = getNewYear(date, 1);
                dayNumber = normalizeDay(year.getDay() - DateHelper.weekStartDay);
                weekNumber = dayNumber < 4 ? 1 : 53;
            }
        }
        else {
        ...
        return weekNumber > 0 ? [year.getFullYear() - 1 + yearDelta, weekNumber] : DateHelper.getWeekNumber(date, weekStartDay, -1);
    }
    ...
    static applyLocale() {
        ...
        if (locale === 'en-US') {
            ...
            ordinalSuffix = me.L('L{ordinalSuffix}') || ordinalSuffix;
        }
