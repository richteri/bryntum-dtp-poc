...
export default class BryntumWidgetAdapterRegister {
    static register(type, widgetClass) {
        const classEntry = widgetClassRegister.get(widgetClass);
        ...
        if (classEntry) {
            ...
            widgetClassRegister.set(widgetClass, {
                [type] : 1
            });
        }
    }
     ...
     * @param {String} type The type string to test against.
    ...
    static isType(widget, type, deep) {
        ...
        for (let widgetClass = widget.constructor; widgetClass; widgetClass = deep && Object.getPrototypeOf(widgetClass)) {
            ...
            const
                ...
                name = Object.prototype.hasOwnProperty.call(widgetClass, '$name') ? widgetClass.$name : widgetClass.name;

            let classEntry = widgetClassRegister.get(widgetClass);
            if (!classEntry && name) {
                classEntry = {
                    [name.toLowerCase()] : 1
                };
                widgetClassRegister.set(widgetClass, classEntry);
            }
...
const
    ...
    widgetClassRegister = BryntumWidgetAdapterRegister.widgetClassRegister = new Map();
