...
export default class AjaxStore extends Store {
    ...
    commit() {
        ...
        // not allowing additional commits while in progress, TODO: should queue
        // If no outstanding changes, or we are already committing - return a Promise that resolves immediately.
        if (!changes || me.commitPromise) {
    ...
    commitModified(commitState) {
        ...
        /**
         * modified set contains all records that has been changed include records where only `persist: false` flagged
         * fields has been changed. We should clean it up and check if still something to commit is left.
         */
        modified.forEach(record => {
            const modificationData = record.modificationData;
            if (!ObjectHelper.isEmpty(modificationData)) {
                modifications.push(me.writeAllFields ? record.persistableData : modificationData);
            }
        });

        if (!modifications.length || !me.updateUrl) {
            return null;
        }

        const event = { action : 'update', params : me.params };
