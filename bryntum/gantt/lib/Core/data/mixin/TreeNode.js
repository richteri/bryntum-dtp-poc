...
export default Target => class TreeNode extends (Target || Base) {
    ...
    updateChildrenIndex(children, unfiltered = false) {
        ...
        for (let i = 0; i < children.length; i++) {
            ...
            //region Update `modified` state which is used in sync request
            //<debug>
            // WARNING!!!
            // This `parentIndex` update is a workaround for Grid and Scheduler.
            // `parentIndex` has to be updated in Model.inSet method.
            // It works in Gantt because `parentIndex` is defined as a field in TaskModel.
            // TODO: https://github.com/bryntum/bryntum-suite/issues/1613
            //</debug>
            if (indexName === 'parentIndex' &&
                !child.getFieldDefinition(indexName) &&
                oldValue !== undefined &&
                oldValue !== child[indexName]
            ) {
                let shouldUpdate = false;
                if (child[indexName] === child.meta.modified[indexName]) {
                    // Remove changes if values are the same
                    Reflect.deleteProperty(child.meta.modified, indexName);
                    shouldUpdate = true;
                }
                else if (!(indexName in child.meta.modified)) {
                    // Cache its original value
                    child.meta.modified[indexName] = oldValue;
                    shouldUpdate = true;
                }

                if (shouldUpdate) {
                    child.stores.forEach(store => {
                        store.updateModifiedBagForRecord(child);
                    });
                }
            }
            //endregion
