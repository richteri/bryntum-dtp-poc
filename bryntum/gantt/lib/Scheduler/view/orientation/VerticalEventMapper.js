...
export default class VerticalEventMapper extends Delayable(Base) {
    ...
    getElementsFromEventRecord(eventRecord) {
        const
            // The syncIdMap has { id => element } mappings for all direct children rendered, for foregroundCanvas
            // that means all events (and similar)
            { syncIdMap }          = this.scheduler.foregroundCanvas,
            { eventInnerSelector } = this.scheduler,
    ...
    toggleCls(eventRecord, resourceRecord, cls, add = true) {
        ...
        if (eventData) {
            ...
            // To be safe, retrieve using `getElementFromEventRecord`
    ...
    addTemporaryDragElement(eventRecord) {
        const
            { scheduler } = this,
            tplData       = scheduler.generateTplData(eventRecord, eventRecord.resource, { timeAxis : true, viewport : true });
        ...
        const domConfig = this.renderEvent({ tplData }),
            { dataset } = domConfig;
    ...
    renderResource(resourceRecord) {
        ...
        for (const eventId in resourceEntry) {
            const
                ...
                { endDateMS, startDateMS, eventRecord } = eventData.tplData;
