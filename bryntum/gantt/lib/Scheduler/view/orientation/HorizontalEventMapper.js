...
const
    getStartEnd = (view, timeAxis, date, dateMS, useEnd) => {
        if (view.fillTicks) {
...
export default class HorizontalEventMapper extends BaseHorizontalMapper {
    ...
    calculateRowHeight(resourceRecord) {
        ...
        if (scheduler.eventLayout === 'stack' && scheduler.eventStore.count) {
            const
                ...
                // sp reapply the EventStore's filterFunction here. If it's not filtered it's ()=>true
                eventFilter     = e => eventStore.storage.filterFunction(e) && timeAxis.isTimeSpanInAxis(e),
                events          = eventStore
                    .getEventsForResource(resourceRecord)
                    .filter(eventFilter)
    ...
    updateRowTimeSpans(row, resource, forceLayout = false, fromHorizontalScroll = false) {
        ...
        if ((scheduler.forceLayout || forceLayout || !resourceLayoutCache) && !me.layoutEvents(scheduler, resource, row)) {
            return resourceLayoutCache && resourceLayoutCache._rowHeight;
        }

        // might have been updated above
        resourceLayoutCache = cache.getRow(resourceId);

        if (!resourceLayoutCache) {
            return null;
        }
    ...
    onEventDataset() {
        ...
        // Force RowManager to calculate a new average row height. This adds some domain knowledge from Scheduler, when
        // setting a new dataset on the EventStore rows are affected in a way it is not aware of.
        //
        // Especially important for the first dataset, if resources was set first since then it will have calculated
        // based on empty rows.
    ...
    onEventAdd({ records, resources }) {
        ...
        // Also invalidate cached row height, once, at the end, for each unique resource affected.
        scheduler.calculateRowHeights([...affectedResources]);
    }

    onEventRemove({ resources }) {
        ...
        // Also invalidate cached row height, as it might no longer be correct
    ...
    onEventUpdate({ source : eventStore, record : eventRecord, changes = {}, resources }) {
        ...
        if (resourceWasChanged) {
            const
                oldResource              = eventStore.resourceStore.getById(changes.resourceId.oldValue),
            ...
            // Invalidate cached row height, as it might no longer be correct
        ...
        // start or end date changed, need to update layout
            ...
            // Invalidate cached height. Really only needed if not in view, but not worth adding that logic here
        ...
        resources.forEach(resourceRecord => {
            const
                eventLayoutData = me.getTimeSpanRenderData(eventRecord, resourceRecord),
