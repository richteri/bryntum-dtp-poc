...
export default Target => class SchedulerStores extends (Target || Base) {
    ...
    onStoreAdd({ source : store, records, isChild, isExpandAll }) {
        const me             = this,
            { rowManager } = me;
    ...
    onInternalEventStoreChange(params) {
        ...
        if (me.isVertical) {
            ...
            if (!me.isPainted) {
                // Not ready to draw yet, but needs to determine row heights anyway to get initial height correct
                if (params.action === 'dataset') {
                    // Silent call, to not trigger height changes too early
